...
def get_search_value():
    ...
    while True:
        user_input = (
            input(f"Введите {search_elem} для поиска сотрудника: ").strip().lower()
        )
...
def replay():
    while True:
        replay = input(f"Хотите возобновить поиск сотрудника? (y/n): ").strip().lower()
...
while True:
    add_agree = input("Хотите добавить нового сотрудника? (y/n): ").strip().lower()
    ...
    if add_agree in ["yes", "нуы", "y", "н"]:
        ...
        while not surname or not re.fullmatch(r"[а-яёa-z-]+", surname, re.IGNORECASE):
            surname = (
                input("Введите корректную фамилию (только буквы и '-'): ")
                .strip()
                .title()
            )
        ...
        while not name or not re.fullmatch(r"[а-яёa-z-]+", name, re.IGNORECASE):
            name = (
                input("Введите корректное имя (только буквы и '-'): ").strip().title()
            )

        last_name = input("Отчество: ").strip().title()
        while not last_name or not re.fullmatch(
            r"[а-яёa-z-]+", last_name, re.IGNORECASE
        ):
            last_name = (
                input("Введите корректное отчество (только буквы и '-'): ")
                .strip()
                .title()
            )
        ...
        skills = (
            [s.strip().title() for s in skills_input.split(",")] if skills_input else []
        )
        ...
        while True:
            ...
            if has_children in ["no", "тщ", "n", "т"]:
                ...
                while not child_name or not re.fullmatch(
                    r"[а-яёa-z-]+", child_name, re.IGNORECASE
                ):
                    child_name = (
                        input("Введите корректное имя ребёнка (только буквы и '-'): ")
                        .strip()
                        .title()
                    )
                ...
                while True:
                    try:
                        ...
                        if (
                            1900 <= child_birth <= 2025
                            and child_birth <= birth_year - 14
                        ):
                            break
                        else:
                ...
                childrens.append({"name": child_name, "birth_year": child_birth})

                more = input("Добавить ещё одного ребёнка? (да/нет): ").strip().lower()
