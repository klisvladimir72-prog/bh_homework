...
# пасим оценки по предметам


def parse_grades(grades_str: str):
    subj = grades_str.split('), ')

    result = {}
    for part in subj:
        subj_name, grades = part.strip().split('(')

        grades = grades[:-1]
        try:
            grades_list = [int(x.strip()) for x in grades.split(',')]
        except ValueError:
            continue

        result[subj_name.strip()] = grades_list
